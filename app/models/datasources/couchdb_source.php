 ...
 */
	public function create($model, $fields = null, $values = null) {
		...
		if (isset($data[$model->primaryKey]) && !empty($data[$model->primaryKey])) {
			...
			$uuids = $this->decode($this->Socket->get('/_uuids'));
			$params = $uuids->uuids[0];
		}

		$result = $this->decode($this->Socket->put($this->uri($model, $params), $this->encode($data)));
 ...
 */
	public function update($model, $fields = null, $values = null, $conditions = null) {
		...
		$this->idRevData($model, $data);

		if (!empty($model->id)) {
			$result = $this->decode($this->Socket->put($this->uri($model, $model->id), $this->encode($data)));
			if ($this->checkOk($result)) {
				$model->rev = $result->rev;
				return true;
			}
 ...
 * @return void
 */
	private function idRevData(&$model, &$data) {
		if (isset($data[$model->primaryKey]) && !empty($data[$model->primaryKey])) {
 ...
 */
	public function query($uri, $post) {
		return $this->decode($this->Socket->post($uri, $this->encode($post)));
	}
